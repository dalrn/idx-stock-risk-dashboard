...
sector_avg = (
    ...
    .agg(
        Avg_Score=("Stability_Score", "mean"),
...
# Tampilkan 3 insight random (stabil per session)
if "insight_indices" not in st.session_state:
    rng = np.random.default_rng()
    st.session_state.insight_indices = rng.choice(
        len(insights), size=min(3, len(insights)), replace=False
    ).tolist()

cols = st.columns(3)
for i, idx in enumerate(st.session_state.insight_indices):
    ins = insights[idx]
    with cols[i]:
        st.markdown(
            f"""
            ...
            ">
                ...
                <div style="font-weight: 600; margin-bottom: 8px;">{ins['title']}</div>
                <div style="font-size: 0.9em; color: rgba(128,128,128,0.9);">{ins['body']}</div>
            </div>
            """,
            unsafe_allow_html=True,
        )

if st.button("🔄 Tampilkan insight lain", type="secondary"):
    rng = np.random.default_rng()
    st.session_state.insight_indices = rng.choice(
        len(insights), size=min(3, len(insights)), replace=False
    ).tolist()
